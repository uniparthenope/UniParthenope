...
class AndroidDebugBridgeResultHandler {
    constructor($logger, $errors) {
        this.$logger = $logger;
        this.$errors = $errors;
    }
    checkForErrors(adbResult) {
        const errors = [];
        if (adbResult) {
            if (_.isArray(adbResult)) {
                _.each(AndroidDebugBridgeResultHandler.ANDROID_DEBUG_BRIDGE_ERRORS, (error) => {
    ...
    handleErrors(errors, treatErrorsAsWarnings) {
        _.each(errors, (error) => {
            this.$logger.trace(`Error name: ${error.name} result code: ${error.resultCode}`);
        });
        const errorMessages = _(errors).map((error) => error.description)
            .join(os_1.EOL)
            .toString();
